...
int64_t SumArraySimd(const int64_t *data, size_t length) {
    ...
    auto block_sums = _mm256_extract_epi64(sum_block, 0) + _mm256_extract_epi64(sum_block, 1) +
                      _mm256_extract_epi64(sum_block, 2) + _mm256_extract_epi64(sum_block, 3);
...
int64_t SumArraySisd(const int64_t *data, size_t length) {
    int64_t sum = 0;
