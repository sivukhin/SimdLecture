...
- Современные процессоры Intel обычно поддерживают такие SIMD расширения как SSE4.1/SSE4.2, AVX и даже AVX 2.0
...
- AVX вводит 256-битные регистры, различные виды кодирования (VEX coding scheme), но операций с 256-битными регистрами мало
- AVX 2.0 уже имеет нормальный набор инструкций для 256-битных регистров
...
- Что нужно, чтобы работать с SIMD в C#?
	- RyuJIT Jitter. В C#, CIL нет никаких специальных векторных инструкций, поэтому они внедряются в код приложения на этапе JIT-компиляции. RyuJIT знает о классах пространства имен System.Numerics.Vectors и транслируют CIL, относящийся к ним, в специальные векторные ассемблерные инструкции
	- .NET версии >= 4.6
